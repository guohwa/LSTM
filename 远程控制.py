...
# 服务器配置，根据您的需求修改
server_list = [
    {"name": "vscode-001", "ip": "46.8.226.41", "port": 22, "username": "root", "password": "Gh@8439033", "remote_path": "/home/"},
...
# 本地目录路径，根据您的需求修改
local_path = r"D:\014_tradingview\python\LSTM-for-cryptocurrency-forecasting-main\LSTM-for-cryptocurrency-forecasting-main"
...
def execute_command_on_remote(server, command):
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
...
# 执行命令到每个远程服务器
command = input("请输入要在远程服务器上执行的命令：")
for server in server_list:
    execute_command_on_remote(server, command)
