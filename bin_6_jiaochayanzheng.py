...
def train_and_predict():
    ...
    # 生成交易信号
    price_diff = next_value[0][0] - raw_current_price
    if abs(price_diff) / raw_current_price > threshold:
        if price_diff > 0:
            signal = "买入信号"
        else:
            signal = "卖出信号"
        ...
        if current_order is not None:
            print(f"取消前一个订单: {current_order}")
            # 这里可以添加取消订单的逻辑
            current_order = None

        # 执行新订单
        current_order = signal
        print(f"执行新订单: {current_order}")
        # 这里可以添加执行订单的逻辑
    else:
        signal = "无交易信号"
        print(f"交易信号: {signal}")
    ...
    body = f"""
    <h2>BTC 预测结果</h2>
    <p>BTC 当前价格: {raw_current_price:.2f}</p>
